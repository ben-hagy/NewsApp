...
/**
 * Searching is handled primary through the addTextChangedListener and the lambda following that.
 * We save a coroutine job in an empty variable, then cancel any current job assigned to it for
 * safety. Then, in a coroutine, we wait for 500ms (saved as a const val in our helper class), then
 * let our "editable" lambda object be populated with the query typed into the etSearch field.
 *
 * The short delay keeps us from unnecessarily pinging the API too many times.
 */
...
class SearchNewsFragment : Fragment(R.layout.fragment_search_news) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // handles bundling Article class data and sending it alongside nav action to articleFragment
